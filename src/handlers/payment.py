...
async def successful_payment_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    # Важно: service_ и gift_ обрабатываются отдельными хендлерами.
    # Если здесь дублировать логику и/или делать state.clear(), можно потерять данные FSM
    # и получить двойные записи в БД.
    if payload.startswith("service_") or payload.startswith("gift_"):
        return
...
async def _process_diagnostic_payment(message: Message, payment, state: FSMContext, bot: Bot):
    ...
    await message.answer(
        ...
        "Отправьте два фото:\n\n"
        "📸 *Фото 1:* лицом в полный рост, глаза смотрят в объектив (строго без очков).\n"
        "Желательно на нейтральном светлом фоне.\n\n"
        "_Фото нужны только для диагностики и после удаляются.\n"
        "Гарантированно без передачи третьим и иным лицам._",
        parse_mode="Markdown"
    )
