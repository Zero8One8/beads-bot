...
async def admin_site_rollback(callback: CallbackQuery):
    ...
    # history_id is a UUID (last segment after final _), content_key may contain _
    # Split from the right: last 5 parts are UUID (8-4-4-4-12)
    parts = rest.rsplit("_", 5)
    if len(parts) < 6:
        await callback.answer("❌ Неверный формат callback", show_alert=True)
        return
    content_key = parts[0]
    history_id = "_".join(parts[1:])
