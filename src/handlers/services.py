...
async def service_detail(callback: CallbackQuery, state: FSMContext):
    ...
    # Поддержка и service_{id}, и service_book_{id}
    try:
...
@router.message(F.successful_payment.invoice_payload.startswith("service_"))
...
async def booking_cancel_draft(callback: CallbackQuery, state: FSMContext):
    """Отмена записи до оплаты (кнопка подтверждения)."""
    await state.clear()
    await callback.message.edit_text(
        "❌ Запись отменена.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
